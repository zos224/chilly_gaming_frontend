<template>
    <div>
        <div class="bg-slate-50 p-5 rounded-xl flex justify-between items-center">
            ...
            <span>
                <button @click="router.push({name: 'AddArticle'})" class="flex bg-green-500 text-white font-semibold px-3 py-2 rounded-lg">
        ...
        <div class="mt-5 bg-slate-50 p-5 rounded-xl ">
            <h4 class="text-center font-semibold text-lg">Danh sách tin tức trong hệ thống</h4>
            ...
            <div class="mt-8" style="min-height: max-content;">
                <div v-if="articles">
                    <table id="tableArticle" class="table-auto w-full border-collapse border-2 ">
                        <tr class="border-2">
                            <th class="border-2 ">ID Bài Viết</th>
                            <th class="border-2">Tiêu Đề Bài Viết</th>
                            <th class="border-2">Thumbnail Bài Viết</th>
                            <th class="border-2">Đánh Giá</th>
                            <th class="border-2">Lượt Xem</th>
                            <th colspan="2" class="border-2">Tùy chọn</th>
                        </tr>
                        <tr class="text-center border-2" v-for="article in articles" :key="article.id">
                            <td class="border-2">{{ article.id }}</td>
                            <td class="border-2">{{ article.title }}</td>
                            <td class="border-2"><img class="w-24 h-24 object-cover m-auto" :src="'https://chillygamingbackend-production.up.railway.app/images/articles/' + article.thumb_image" alt=""></td>
                            <td v-if="article.rate == 0" class="border-2">{{ article.rate }}</td>
                            <td v-else>
                                <div class="flex justify-center">
                                    <span>{{ article.rate }} </span>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="#FFCC00" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                                    </svg>
                                </div>
                            </td>
                            <td class="border-2">{{ article.luotxem }}</td>
                            <td>
                                <button @click="router.push({name: 'UpdateArticle', params: {id: article.id}})">
                            ...
                            <td>
                                <button @click="delArticle(article.id)">
...
<script setup>
    ...
    let optionsSort = [
        {name: 'ID', value: 'id'},
        {name: 'Tiêu Đề Bài Viết', value: 'title'},
        {name: 'Đánh Giá', value: 'rate'},
        {name: 'Lượt Xem', value: 'luotxem'}
    ]
    ...
    function changeData()
    {
        ...
        store.dispatch('getArticlePageSort', require).then((res) => {
    ...
    function delArticle(id)
    {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                store.dispatch('delArticle', id).then(() => {
                    Swal.fire(
                    'Đã xóa!',
...
<style>
    #tableArticle tr:nth-child(even) {
